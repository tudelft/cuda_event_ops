...
torch::Tensor trilinear_splat_forward(torch::Tensor points, int grid_d, int grid_h, int grid_w,
                                      int threads, int points_per_thread) {
    CHECK_INPUT(points);
    auto grid = torch::zeros({points.size(0), grid_d, grid_h, grid_w}, points.options());

    return trilinear_splat_cuda(points, grid, grid_d, grid_h, grid_w, threads, points_per_thread);
}

torch::Tensor trilinear_splat_backward(torch::Tensor grad_output, torch::Tensor points, int grid_d,
                                       int grid_h, int grid_w, int threads, int points_per_thread) {
