...
class Iterative3DWarpCuda(torch.autograd.Function):
    ...
    def forward(ctx, points, flow_fields, num_warps, keep_warping, num_backprop_points, threads, points_per_thread):
        warped_points = iterative_3d_warp_forward(
            points, flow_fields, num_warps, keep_warping, threads, points_per_thread
        )
...
def iterative_3d_warp_cuda(
    points, flow_fields, num_warps, keep_warping, num_backprop_points, threads=1024, points_per_thread=1
):
    return Iterative3DWarpCuda.apply(
        points, flow_fields, num_warps, keep_warping, num_backprop_points, threads, points_per_thread
    )
...
class TrilinearSplatCuda(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_points = trilinear_splat_backward(
            grad_output.contiguous(), points, *grid_resolution, threads, points_per_thread
        )
